...
class UploadHandler
{
    ...
    protected $error_messages = array(
        ...
        'max_file_size' => '文件过大',
        'min_file_size' => '文件过小',
        'accept_file_types' => '不允许的文件格式',
        'max_number_of_files' => '超过同时上传数量限制',
        'max_width' => '超过图片最大宽度限制',
        'min_width' => '不足图片最小宽度限制',
        'max_height' => '超过图片最大高度限制',
        'min_height' => '不足图片最小高度限制'
    );

    function __construct($options = null, $initialize = true, $error_messages = null) {
        $user_folder_name = $this->get_user_id();
        $this->options = array(
            'script_url' => $this->get_full_url().'/source/upload_ajax',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/uploads/',
            'upload_url' => $this->get_full_url().'/uploads/',
            'store_dir' => 'uploads/', //by hills for file exit check
            'user_dirs' => true,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    protected function get_user_id() {
        if($_SESSION['ID']){
            return 'u_'.$_SESSION['NAME'].'_id'.$_SESSION['ID'];
        } 
        return session_id();
    }
    ...
    protected function get_download_url($file_name, $version = null) {
       ...
       // return $this->options['upload_url'].$this->get_user_path()
        //    .$version_path.rawurlencode($file_name);
         //return $this->options['store_dir'].$this->get_user_path()
       //     .$version_path.rawurlencode($file_name);
	   return $this->options['store_dir'].$this->get_user_path()
            .$version_path.rawurlencode($file_name);
        //by hills for
    ...
    protected function create_scaled_image($file_name, $version, $options) {
        ...
        if (!empty($version)) {
			...
			$file_name = iconv("utf-8","gbk",$file_name); //add by hills
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                  ...
                  //  move_uploaded_file($uploaded_file,iconv("utf-8","gbk",$file_path));
    ...
    protected function get_file_name_param() {
        return isset($_GET['file']) ? trim(basename(' ' . stripslashes($_GET['file'])), ".\x00..\x20") : null;
    }
